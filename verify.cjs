...
function orderTable(mobile) {
  return `<table class="order-table ${mobile ? 'mobile' : 'desktop'}"><tbody><tr><th></th>${days.map(d => `<td>${d}</td>`).join('')}</tr>` +
    options.map(([meal, variant]) => `<tr><td>Option ${variant} (6.50€)</td>${days.map((_, i) => `<td><div class="form-check"><input type="checkbox" class="order-checkbox" name="2026-09-${String(7 + i).padStart(2, '0')}_${meal}_${variant}" ${i === 0 ? 'checked' : ''} ${i === 4 ? 'disabled' : ''}></div></td>`).join('')}</tr>`).join('') + '</tbody></table>';
}
...
setTimeout(() => {
  ...
  check(visible.parentElement.scrollWidth > visible.parentElement.clientWidth, 'table scrolls horizontally');
